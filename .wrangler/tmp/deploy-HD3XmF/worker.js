...
var USER_COLLECTION = "vimflowy_users";
...
async function directus(env, path, options = {}) {
  ...
  if (!base)
    throw new Error("DIRECTUS_STATIC_URL not configured");
  const token = env.DIRECTUS_STATIC_TOKEN;
  if (!token)
    throw new Error("DIRECTUS_STATIC_TOKEN not configured");
  ...
  if (!response.ok) {
    ...
    throw new Error(`Directus ${response.status}: ${text2}`);
  }
...
__name(upsertValue, "upsertValue");
async function handleLogin(body, env) {
  ...
  const response = await directus(
    env,
    `/items/${USER_COLLECTION}?filter[username][_eq]=${encodeURIComponent(username)}&filter[password_hash][_eq]=${encodeURIComponent(hash)}&fields=id,username,display_name,role&limit=1`
  );
...
__name(requireUserFromRequest, "requireUserFromRequest");
...
async function handleDataSetMany(request, env) {
  ...
  const safeEntries = entries.filter((entry) => entry && typeof entry.key === "string" && typeof entry.value === "string");
  await Promise.all(safeEntries.map((entry) => upsertValue(user.id, entry.key, entry.value, env)));
  return json({ ok: true });
}
__name(handleDataSetMany, "handleDataSetMany");
function setupSocket(server, env) {
  ...
  __name(sendCallback, "sendCallback");
  server.addEventListener("message", async (event) => {
    try {
      ...
      if (msg.type === "join") {
        ...
        if (!user)
          return sendCallback(msg.id, { error: "Invalid or expired auth token" });
      ...
      if (msg.type === "setMany") {
        ...
        await Promise.all(
          entries.filter((entry) => typeof entry?.key === "string").map((entry) => upsertValue(joinedUserId, entry.key, entry.value, env))
        );
        return sendCallback(msg.id, { error: null });
      }
